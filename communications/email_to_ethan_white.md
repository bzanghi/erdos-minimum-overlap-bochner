...
an improvement of about `+5.4 × 10⁻⁴` over your published value, with
the same proof structure, just slightly enlarged ellipses.

A note on what's NOT in this number: an earlier intermediate result
extended to `µ ≥ 0.379828` (+8.2e-4) by adding a Lasserre level-2 SDP
on the bilinear `|f̂(m)|²` products. Adversarial review flagged that my
Lasserre-2 implementation truncates the moment expansion
`(f²)̂(m) = Σ_n f̂(n)f̂(m−n)` to `|n| ≤ T_max` without rigorously
bounding the tail. I worked out the natural Fejér-Riesz / Parseval
rigorization explicitly (notes attached): the math goes through
algebraically, but at the relevant `T_max ≈ 30` the closed-form tail
slack is on the same order as the `Loc` diagonal — about 700× the
heuristic Lasserre gain it was supposed to rescue. So the natural
rigorization **does not produce a useful bound**. Pushing past +5.4e-4
this way appears to require either much larger `T_max` (intractable at
present SDP scale) or a finite-dimensional SOS exactness argument I
don't yet have. The Bochner-PSD result (this +5.4e-4) stands cleanly
as a verified ceiling for the Bochner-only approach.
