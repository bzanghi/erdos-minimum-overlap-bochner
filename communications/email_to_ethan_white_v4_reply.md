...
Thank you for the generous reply — and especially for the two corrections. Since my program is a direct transcription of your Section 5, both errata pointed straight at specific lines of the code, and chasing them down was immediately useful:

**On (5.8)/(5.9):** my implementation already used 2m−1 in the right-hand sides — the transcription took the index from the left-hand side δ_{2m−1}/ε_{2m−1}, so this erratum turns out not to affect it (the corrected form is the looser one, so it's the valid direction in any case).

**On (5.6)/(5.7):** here I had faithfully inherited the paper's 8. Re-solving at the binding center (your row 4, with the Bochner-PSD augmentation at n=20) with the corrected 4, the optimum moves by only +1.8 × 10⁻⁶ — consistent with your "not material" finding, and in the fortunate direction: the corrected program is (very slightly) *tighter*, so the values I've been reporting remain valid lower bounds, and re-running the full pipeline with the 4 should marginally improve them. That full re-run is queued; I'll use the corrected constant everywhere going forward.

**On average-values-of-f variables:** I think your instinct about why the PSD constraint helps is exactly right. The moment matrix encodes "f and 1−f are nonnegative" spectrally, with no new variables beyond the existing Fourier coefficients — it's the Bochner/Herglotz criterion truncated to an n×n principal minor. I did also test the interval-average route indirectly: a diagnostic that projects the best known upper-bound construction into your variable space shows the f-side constraints (including the PSD blocks) are healthily slack there, and essentially all of the relaxation gap sits in the cell-kernel envelope on the M side — which suggests average-f variables would add little beyond what the PSD blocks capture, and that the M-side envelope is where the framework's slack lives.
...
2. Possibly the more interesting development: a computable ceiling for this technique class — using the measured dual multipliers of the cell-envelope family, no augmentation of the cosine+sine envelope can prove beyond ≈ 0.38071 at any tested scale. And on the construction side, Together's step function turns out to be a numerically exact KKT point of the discretized min-max problem (stationarity residual ~10⁻⁸), and it stays stationary — to first *and* second order — when the grid is refined from 600 to 1200 cells. (I initially concluded the opposite from a mis-derived stationarity equation that dropped the domain-edge terms; the corrected test reversed it.) Taken together: 0.380871 looks like a serious candidate for µ itself, and if that's right, closing the gap from below provably requires something outside this convex-relaxation framework. Do you have any instinct on whether the true extremizer should be expected at such a "flat" degenerate optimum — the active set of shifts has positive density — or is that degeneracy itself suspicious to you?

I should be upfront about who you're corresponding with: I'm a software engineer, not a mathematician, and there's no research program or institution behind this — it's a personal project done out of intellectual curiosity, with much of the heavy lifting (the derivations, the implementations, the diagnostics) done in close collaboration with an AI assistant (Anthropic's Claude). I mention it partly for honesty and partly because it shapes the verification culture: since I can't referee the mathematics by eye, everything load-bearing is cross-checked mechanically — independent re-implementations agreeing to 10+ digits, multiple solvers (CLARABEL/Mosek/SDPA-GMP), and dual certificates rather than primal values. The mis-derived stationarity equation I mentioned above is a fair example of the failure mode this workflow has to guard against — and of the checks catching it.

I'm writing this up as a short note (the bound plus the ceiling theorem), with the AI-assisted workflow disclosed in detail. I'd be glad to send you the draft and the research notes, and happier still if you wanted to weigh in on the framing — with co-authorship open if any of it grows into something you'd want your name on, and a grateful acknowledgment otherwise.
...
- Kept to ~700 words; the complementarity question from v3 is *dropped* here to keep the reply focused — it can go in a follow-up or the note itself.
- Per Ben: the "who you're corresponding with" paragraph is deliberate — engineer + AI, personal curiosity project, no program/institution. Keep it even if trimming elsewhere; it sets expectations honestly and explains the mechanical-verification culture.
