...
**Headline claim (rigorous, independently verified):**
$$\boxed{\mu \;\ge\; 0.379544}, \quad \text{an improvement of } +5.4 \times 10^{-4} \text{ over White (2023)'s } 0.379005.$$

This uses Bochner moment-matrix PSD constraints added to White's Section-5 program plus White's own §5.1 / Appendix II ellipse-extension argument. Three independently-written code paths agree to 10+ digits on the per-row SDPs; an independent re-encoding of the Bochner constraint by a separate agent gives bit-for-bit agreement.

**Retracted heuristic claim:** an earlier extended bound `µ ≥ 0.379828` (Bochner + Lasserre level-2, claimed +8.2 × 10⁻⁴) was retracted on adversarial review. The Lasserre-2 implementation in `code/lasserre.py` truncates the polynomial moment expansion `(f²)̂(m) = Σ_n f̂(n)f̂(m−n)` to `|n|, |m-n| ≤ T_max` *without bounding the omitted tail*, making the resulting constraint a heuristic test rather than a proven valid inequality (cf. the file's own comment: "treating the result as a heuristic test"). An independent re-encoding in a complex Fourier basis gave essentially zero Lasserre contribution at the test config, while the existing implementation at large `T_max` reports a positive contribution that is plausibly an artifact of the silently-dropped tail terms.

**Update (post-tail-bound analysis):** the natural Fejér-Riesz / Parseval tail bound was derived in `communications/lasserre_tail_bound.md`. The math goes through algebraically, but quantitatively it kills the gain: at White's parameters `T_max = T_loc = 30`, the Toeplitz/Gershgorin operator-norm bound on the truncation slack is `‖Δ‖_op ≲ 0.19`, **the same order as the Loc matrix diagonal** `Ω/2 ≈ 0.19` and ~700× the heuristic Lasserre contribution +2.8 × 10⁻⁴. The Hilbert-Schmidt refinement still falls short by ~100×. **Conclusion: the natural rigorization of the Lasserre-2 augmentation does not produce a useful rigorous bound.** Pushing beyond the verified +5.4 × 10⁻⁴ requires a fundamentally new lever, not just rigorizing the Lasserre extension. Specifically, future work would need much larger `T_max` (>1000) or a finite-dimensional SOS exactness argument — neither tractable in our SDP scale. This makes `µ ≥ 0.379544` not just our current best but the verified ceiling for Bochner-PSD + ellipse-extension family of techniques on White's program at currently-tractable SDP scale.
